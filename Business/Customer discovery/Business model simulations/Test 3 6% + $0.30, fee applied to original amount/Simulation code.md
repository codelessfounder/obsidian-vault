...
```javascript
/**
 * Discopay Transaction Simulation – 100 Transactions (Fee only on Basket Price)
 *
 * Fee: 6% + $0.30 charged only on the $100 base basket value (not overpayments)
 * Merchants receive: $100 - fee
 */

const transactions = 100;
const basketValue = 100;
const feeRate = 0.06; // New 6% fee
const fixedFee = 0.30; // Flat fee per transaction
const fee = basketValue * feeRate + fixedFee; // Fee always calculated from basketValue
const merchantReceives = basketValue - fee;
