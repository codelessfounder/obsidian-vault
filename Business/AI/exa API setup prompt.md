...
## 🦞 Exa MCP Server for OpenClaw

Give OpenClaw real-time web search, code context, and company research with Exa MCP.

**Recommended — Native web search provider** ([docs](https://docs.openclaw.ai/tools/exa-search))

Make Exa the built-in search engine for every OpenClaw agent. This is the best integration — every `web_search` call routes through Exa natively with zero extra config per agent. Set `EXA_API_KEY` in `~/.openclaw/.env` and run `openclaw configure --section web`. Manage keys at [dashboard.exa.ai/api-keys](https://dashboard.exa.ai/api-keys).

**Alternative — MCP client registry** ([docs](https://docs.openclaw.ai/cli/mcp))

If you only need Exa as an optional tool (rather than the default search provider), register it via MCP:

```bash
openclaw mcp set exa --url https://mcp.exa.ai/mcp
```
